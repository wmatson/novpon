...
function App() {
  ...
  const [sentence, setSentence] = useState(''); const [guess, setGuess] = useState(''); const [results, setResults] = useState<GradeResult[]>([]); const [busy, setBusy] = useState(false); const [message, setMessage] = useState('');
  useEffect(() => { const listener = () => { setRoute(routeFromHash()); setResults([]); setGuess(''); setMessage(''); }; addEventListener('hashchange', listener); return () => removeEventListener('hashchange', listener); }, []);
  useEffect(() => { if (route.kind === 'custom') { try { setSentence(decodeSentence(route.encoded)); } catch { setMessage('That shared puzzle link is not valid.'); } } }, [route]);
  ...
  async function submit(event: Event) { event.preventDefault(); if (!guess.trim() || busy) return; setBusy(true); const result = await gradeGuess(currentPuzzle, guess); setResults(previous => [...previous, result]); setMessage(result.lengthError ?? (result.won ? 'You found it.' : '')); setGuess(''); setBusy(false); }
  return <main className="shell game"><button className="back" onClick={() => go('/')}>← home</button><div className="game-header"><p className="eyebrow">{route.kind === 'daily' ? `DAILY · ${utcDate()}` : route.kind === 'random' ? 'RANDOM VERSE' : 'CUSTOM PUZZLE'}</p><h2>Guess the sentence</h2><p>Use exactly <b>{puzzle.wordCount}</b> words. Meaning earns closeness; exact words win.</p></div><div className="guess-list">{results.map((result, index) => <div className="attempt" key={index}>{result.lengthError ? <p className="error">{result.lengthError}</p> : result.feedback.map((item, wordIndex) => <span className={`token ${item.category}`} key={wordIndex}><b>{item.guess}</b><small>{item.category.replace('-', ' ')}</small></span>)}</div>)}</div>{results.at(-1)?.won ? <section className="success"><span>✦</span><h3>Sentence found</h3><p>Nice work. The target was solved exactly.</p><button onClick={() => go('/')}>Play another</button></section> : <form onSubmit={submit} className="guess-form"><input value={guess} onInput={e => setGuess((e.target as HTMLInputElement).value)} placeholder="Type your guess…" autoFocus /><button disabled={busy}>Guess</button></form>}<p className="message">{message}</p><div className="legend"><span><i className="dot exact" />exact</span><span><i className="dot very-close" />very close</span><span><i className="dot close" />close</span><span><i className="dot far" />far</span><span><i className="dot no-match" />no match</span></div></main>;
}
export default App;
