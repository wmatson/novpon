...
function App() {
  ...
  return <main className="shell game">
    ...
    <div className="guess-list">{results.map((result, index) => <div className="attempt" key={index}><span className="attempt-number">Guess {index + 1}</span>
      {result.lengthError ? <p className="error">{result.lengthError}</p> : result.feedback.map((item, wordIndex) => <span className={`token ${item.category}${item.position === 'wrong' ? ' wrong-position' : ''}`} aria-label={`${item.guess}: ${item.category.replace('-', ' ')}${item.position ? `, ${item.position} position` : ''}`} key={wordIndex}><b>{item.guess}</b><small>{item.category.replace('-', ' ')}{item.position ? ` · ${item.position} position` : ''}</small></span>)}
    </div>)}</div>
    ...
    <p className="message" role="status" aria-live="polite">{message}</p><div className="legend"><span><i className="dot exact" />exact</span><span><i className="dot wrong-position" />wrong position</span><span><i className="dot very-close" />very close</span><span><i className="dot close" />close</span><span><i className="dot no-match" />no match</span></div>
    <a className="github-link" href="https://github.com/wmatson/novpon" target="_blank" rel="noreferrer">View source on GitHub ↗</a>
  </main>;
}
export default App;
